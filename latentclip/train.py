...
if __name__ == '__main__':
    ...
    clip_model = clip_model.visual.eval().to(accelerator.device).to(torch.float32)

    @torch.no_grad()
    def get_gt_clip(tensors):
        return clip_model(clip_preprocess(tensors))

    # define out model
    latent_clip_model = LatentCLIP()
    optimizer = torch.optim.AdamW(latent_clip_model.parameters(), lr=args.learning_rate)
    ...
    latent_clip_model, optimizer, train_loader = accelerator.prepare(latent_clip_model, optimizer, train_loader)
    ...
    for e in range(num_epoch):
        for tensors in train_loader:
            with accelerator.accumulate(latent_clip_model):
                with torch.no_grad():
                    latents = tensor_to_latent(vae, tensors)
                    gt = get_gt_clip(tensors)
            
                pred = latent_clip_model(latents)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % log_freq == 0:
                        ...
                        accelerator.unwrap_model(latent_clip_model).save_pretrained(save_path)
                        ...
                        with torch.no_grad():
                            ...
                            val_gt = get_gt_clip(val_tensors)
                            val_pred = latent_clip_model(val_latents)
